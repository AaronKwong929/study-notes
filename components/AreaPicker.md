...
| 参数    | 说明                                | 类型             | 可选值 | 默认值 |
| :------ | :---------------------------------- | :--------------- | :----- | :----- |
| v-model | 绑定值，地区的对应 code             | Array            | -      | -      |
| columns | 可选择的地区列数，分别为 省/市/区县 | Number           | [1-3]  | 3      |
| width   | 选择器的宽度（默认单位为 px）       | [Number, String] | -      | 360    |
| height  | 选择器的高度（单位为 px）           | Number           | -      | 290    |
...
| 方法名          | 说明                                         | 参数                                               |
| :-------------- | :------------------------------------------- | :------------------------------------------------- |
| getAreaNameList | 获取当前地址 code 对应的中文名，返回一个数组 | 地址 code 数组，如：['110000', '111000', '111001'] |
  ...
  >
    ...
    <section class="area-picker-wrap" :style="{height: height + 'px'}">
      <transition :name="transitionName">
        <section
          class="area-picker-wrap-item"
          v-if="curIndex === 1"
          key="province"
        >
          <el-scrollbar class="area-picker-warp-scrollbar" style="height:100%">
        ...
        <section class="area-picker-wrap-item" v-if="curIndex === 2" key="city">
          <el-scrollbar class="area-picker-warp-scrollbar" style="height:100%">
            <ul class="area-picker-container">
              ...
              <div
                class="area-picker-container-empty"
                v-show="cityList.length === 0"
              >
                暂无数据
              </div>
            </ul>
  ...
  export default {
    ...
    methods: {
      ...
      onPickProvince(province) {
        ...
        this.$emit('pick-province', province.name, province.code);
      ...
      onPickCity(city) {
        ...
        const res = this.formatArea();
        this.$emit('pick', res.areaObjs);
        this.$emit('pick-city', city.name, city.code);
        this.$emit('change', res.codes);
        if (this.curIndex < this.columns) {
          this.transitionName = 'to-left';
          this.curIndex++;
      ...
      onPickCounty(county) {
        ...
        const res = this.formatArea();
        this.$emit('pick', res.areaObjs);
        this.$emit('pick-county', county.name, county.code);
        this.$emit('change', res.codes);
        if (this.curIndex < this.columns) {
          this.transitionName = 'to-left';
          this.curIndex++;
      ...
      formatArea() {
        ...
        for (let i = 0; i < this.columns; i++) {
          ...
          areaObjs[i] = {
            code: codes[i],
            name: area[`${indexs[i]}_list`][+codes[i]],
          };
        }
        return {
          codes,
...
<style lang="scss" scoped>
  .area-picker {
    ...
    &-wrap {
      ...
      overflow-x: hidden;
      position: relative;
      .area-picker-warp-scrollbar ::v-deep .el-scrollbar__view {
        overflow-x: hidden;
      }
